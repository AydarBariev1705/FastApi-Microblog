...
class TestToken:
    ...
    async def test_unidentified_user(self, client: AsyncClient) -> None:
        """
        Тестирование вывода ошибки при запросе с api-key в header,
        но без совпадений в БД
        """
        resp = await client.get("/api/tweets",
                                headers={"api-key": "test-user1000"},
                                )
        er_msg = "Sorry. Wrong api-key token. This user does not exist"
